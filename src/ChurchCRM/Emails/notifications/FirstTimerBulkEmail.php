...
class FirstTimerBulkEmail extends BaseEmail
{
    ...
    public function getTokens(): array
    {
        ...
        return array_merge($this->getCommonTokens(), $this->tokens, [
            ...
            // Reuse configured event reminder logo URL as brand logo slot.
            // This allows using the Good News badge without changing subject/body flow.
    ...
    private function resolveBrandLogoUrl(string $configuredUrl): string
    {
        ...
        if ($configuredUrl === '') {
            ...
            if (is_file($fallbackPath)) {
                $this->mail->addEmbeddedImage($fallbackPath, 'firsttimer-brand-logo-fallback');

                return 'cid:firsttimer-brand-logo-fallback';
            }
        ...
        if ($parts !== false && isset($parts['path'])) {
            ...
            if (str_starts_with($path, '/Images/') && ($isLocalHost || $isSameHost || !isset($parts['host']))) {
                ...
                if (is_file($imagePath)) {
                    $this->mail->addEmbeddedImage($imagePath, 'firsttimer-brand-logo');

                    return 'cid:firsttimer-brand-logo';
                }
            }
        }
